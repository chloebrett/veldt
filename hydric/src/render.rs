...
pub async fn render(demo_option: DemoOption, custom_track: Track, wave: WaveType, volume: Volume, transpose_semitones: i32) -> Vec<f32> {
    ...
    let result = grpc
        .render(RenderRequest {
            track: Some(create_demo_track(demo_option, wave, volume, transpose_semitones).into()),
        })
        .await;
