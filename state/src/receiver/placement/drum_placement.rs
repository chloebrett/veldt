...
impl ActionReceiver for DrumTrackPlacement {
    fn apply(&mut self, action: &Action) -> Option<Action> {
        Some(match action {
            ...
            Action::SetChildById(TypeField::PitchName(pitch_name), TypeField::SampleId(sample_id)) => {
                ...
                if let Some(prev_sample_id) = self.pitch_sample_map.get(&pitch_value) {
                    Action::SetChildById(TypeField::PitchName(*pitch_name), TypeField::SampleId(*prev_sample_id))
                } else {
