...
impl ActionReceiver for OscillatorConfig {
    fn apply(&mut self, action: &Action) -> Option<Action> {
        ...
        Some(match action {
            ...
            // TODO! coarse/fine detune is SUPER WRONG rn prob should only have 1 detune field: detune_cents
            Action::SetFloat(FloatField::Detune, new_detune) => {
                let prev = self.coarse_detune;
                self.coarse_detune = *new_detune;
                Action::SetFloat(FloatField::Detune, prev)
            }
            // Action::SetFloat(FloatField::Detune, new_detune) => {
            //     let prev = self.fine_detune;
            //     self.fine_detune = *new_detune;
            //     Action::SetFloat(FloatField::Detune, prev)
            // }
            ...
            // need to add new float field to differentiate between osc detune and stacking detune
            // Action::SetFloat(FloatField::Detune, new_detune) => {
